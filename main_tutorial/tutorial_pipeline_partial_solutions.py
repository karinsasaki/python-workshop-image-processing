...
# EXERCISE
# Some basic data/image manipulation tools will be used throughout this image  processing and segmentation script, so it makes sense to import them  at the  start.
...
bg = ndi.filters.---(green_smooth,np.---,footprint=struct)     
...
# (i) Label connected components:
     ...
     # Collections of consecutive pixels that have the same value (0 or 1) are considered as connected regions and, if the segmentatin has been done correctly, they correspond to specific objects of the image, for example, different cells. So we label them with integer values using the command ndi.label. (Suggested name for variable 'green_components'). Pick one of the three options below:
#(a)
green_components = ndi.label(green_thresh)[0] 
#(b)
green_components = ndi.label(green_thresh) 
#(c)
green_components = ndi.label(green_thresh)[1] 
...
green_ws = ---(---,---)
    ...
    # Show result as transparent overlay, as we did before. Note that you can use the optional argument alpha of plt.imshow to specify opacity.plt.imshow(---,---,---)
...
for cell_id --- np.unique(green_ws):
    ...
    # If the current cell touches the boundary, remove it (change the cell id to 0)
...
from --- import ---                                          
    ...
    # Save desired array to tif using imsave. Write the correct arguments for the function.
...
# (iv) Write a text file of the numerical data gathered (could be opened e.g. in Excel):
    ...
    # Open an empty file object (using with open), name it and specify the 
    # file format to .txt and the mode to write.
